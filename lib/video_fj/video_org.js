...
function create_video(special_buttons) {
    if (window.location.host != "iperson.top" && window.location.host != "localhost") {
    ...
    html += "</div>";
    html += "<video id='video' class='w-100' style='max-width: 600px; border: 3px solid #000; border-top-left-radius: 5px; border-top-right-radius: 5px;' webkit-playsinline='true' x5-video-player-type='h5' x5-video-player-fullscreen='true'>";
...
function create_control_buttons(special_buttons) {
    ...
    $.each(special_buttons, function (idx, btn_name) {
        ...
        html += "<button id='" + btn_name + "' class='border-0 abs btn btn-outline-light control_btn' " + expand_attr + "><i class='fa fa-" + control_btn_info[btn_name].icon[0] + "'></i></button>";
    });
    return html;
}
...
function process_bar_init() {
    $("#progress").click(function (e) {
        ...
        video_control.exchange_icon($("#play_pause")[0], "fa-play", "fa-pause");
        video_control.play();
...
function process_bar_listener() {
    ...
    if (video.ended) {
        video_control.exchange_icon($("#play_pause")[0], "fa-pause", "fa-play");
