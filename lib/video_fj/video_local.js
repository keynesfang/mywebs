...
function create_video(special_buttons) {
    ...
    html += "</div>";
    html += "<canvas id='video_canvas' class='video_show'></canvas>";
    html += "<video id='video' class='w-100' style='max-width: 600px; display: none;'>";
...
function set_video_vanvas() {
    video.addEventListener('canplaythrough', function () {
        $("#loading").hide();
        $(".video_show").show();
        var videoW = $(video).width();
        var videoH = $(video).height();
        $(video_canvas).attr("width", videoW);
        $(video_canvas).attr("height", videoH);
        var ctx = video_canvas.getContext("2d");
        var img = new Image();
        img.src = "/function/english/video/resources/" + poster + ".png";
